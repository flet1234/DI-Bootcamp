...
const LoginRegister = ({page})=>{
    ...
    const loginregister = async() => {
        if(page === 'Login'){
            try {
                const response = await axios.post('http://localhost:3001/users/login',{email,password},{withCredentials: true})
            ...
            try {
                const response = await axios.post('http://localhost:3001/users/register',{email,password},{withCredentials: true})
